...
public class WishlistPersistenceService implements IWishlistPersistenceService {
  ...
  public Wishlist createWishlist(Long userId) {
    ...
    return wishlistStorageMapper.toModel(
        wishlistRepository.save(wishlistEntity), new CycleAvoidingMappingContext());
  }
  ...
  public Wishlist addProductToWishlist(Long wishlistId, Long productId) {
    WishlistEntity wishlistEntity =
        wishlistRepository
            .findById(wishlistId)
            .orElseThrow(
                () -> new ResourceNotFoundException("Wishlist not found with id: " + wishlistId));
    ...
    boolean productExists =
        wishlistEntity.getItems().stream()
    ...
    if (!productExists) {
      ...
      wishlistEntity.addItem(
          wishlistItemEntity); // Ceci devrait aussi setter wishlistItemEntity.setWishlist(this)
      wishlistRepository.save(
          wishlistEntity); // Sauvegarder la wishlist mettra à jour/créera l'item grâce à
      // CascadeType.ALL
    }
  ...
  public Wishlist removeProductFromWishlist(Long wishlistId, Long productId) {
    ...
    Optional<WishlistItemEntity> itemToRemoveOpt =
        wishlistEntity.getItems().stream()
    ...
    if (itemToRemoveOpt.isPresent()) {
      ...
      wishlistEntity.removeItem(
          itemToRemove); // Gère la liaison bidirectionnelle et orphanRemoval s'occupe de la
  ...
  public Wishlist clearWishlist(Long wishlistId) {
    ...
    wishlistEntity
        .getItems()
        .clear(); // orphanRemoval=true devrait s'occuper de supprimer les WishlistItemEntity de la
    // DB
    // Ou, pour être plus explicite sur la suppression des items orphelins :
    // wishlistItemRepository.deleteAll(wishlistEntity.getItems()); // Cela pourrait être redondant
    // avec orphanRemoval
    ...
    Wishlist model =
        wishlistStorageMapper.toModel(wishlistEntity, new CycleAvoidingMappingContext());
    model.setItems(
        Collections.emptyList()); // Assurer que le modèle retourné reflète la suppression
    return model;
  }
  ...
  public void deleteWishlist(Long wishlistId) {
    ...
    wishlistRepository.deleteById(
        wishlistId); // CascadeType.REMOVE sur UserEntity.wishlist devrait être géré si nécessaire
