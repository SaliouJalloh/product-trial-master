...
public class CartService implements ICartService {
    ...
    private final com.alten.ecommerce.storage.persistence.ICartPersistenceService cartPersistenceService; // Pour accès cart persistence
    ...
    public Cart getCartByUserId(Long userId) {
        ...
        return cartPersistenceService.findByUserId(userId)
                .orElseThrow(() -> new ResourceNotFoundException("Cart not found for user id: " + userId));
    }

    @Override
    public Cart getCartForPrincipal(org.springframework.security.core.userdetails.User springSecurityUser) {
    ...
    @Override
    public Cart addItemToCartForPrincipal(org.springframework.security.core.userdetails.User springSecurityUser, AddItemToCartRequest request) {
        User user = userPersistenceService.findUserByEmail(springSecurityUser.getUsername());
        if (user == null) throw new ResourceNotFoundException("User not found...");
    ...
    @Override
    public Cart updateCartItemForPrincipal(org.springframework.security.core.userdetails.User springSecurityUser, Long itemId, UpdateCartItemRequest request) {
        User user = userPersistenceService.findUserByEmail(springSecurityUser.getUsername());
        if (user == null) throw new ResourceNotFoundException("User not found...");
    ...
    @Override
    public void removeItemFromCartForPrincipal(org.springframework.security.core.userdetails.User springSecurityUser, Long itemId) {
        User user = userPersistenceService.findUserByEmail(springSecurityUser.getUsername());
        if (user == null) throw new ResourceNotFoundException("User not found...");
    ...
    @Override
    public Cart clearCartForPrincipal(org.springframework.security.core.userdetails.User springSecurityUser) {
        User user = userPersistenceService.findUserByEmail(springSecurityUser.getUsername());
        if (user == null) throw new ResourceNotFoundException("User not found...");
