...
public class WishlistService implements IWishlistService {
  ...
  public Wishlist getWishlistForPrincipal(User springSecurityUser) {
    ...
    return wishlistPersistenceService
        ...
        .orElseThrow(
            () -> new ResourceNotFoundException("Wishlist not found for user: " + user.getId()));
  }
  ...
  public Wishlist addProductToWishlist(Long userId, AddProductToWishlistRequest request) {
    log.debug(
        "Attempting to add product ID: {} to wishlist for user ID: {}",
        request.getProductId(),
        userId);
    // Vérifier si l'utilisateur et le produit existent. Lance des exceptions si non trouvés.
    userPersistenceService.findUserById(userId);
    Product product =
        productPersistenceService.findProductById(
            request.getProductId()); // Assure que le produit existe

    // Récupérer ou créer la wishlist
    Wishlist wishlist =
        wishlistPersistenceService
            .findByUserId(userId)
  ...
  public Wishlist removeProductFromWishlist(Long userId, Long productId) {
    log.debug(
        "Attempting to remove product ID: {} from wishlist for user ID: {}", productId, userId);
