    ...
    CartItemEntity itemToUpdate =
        cartItemRepository
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "CartItem not found with id: " + itemId + " in user's cart"));
      ...
      for (var itemModel : cartModel.getItems()) {
        if (itemModel.getProductPrice() != null && itemModel.getQuantity() != null) {
          BigDecimal itemTotalPrice =
              itemModel.getProductPrice().multiply(BigDecimal.valueOf(itemModel.getQuantity()));
