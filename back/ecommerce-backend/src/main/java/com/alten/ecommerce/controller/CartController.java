...
public class CartController {
    ...
    public ResponseEntity<CartResponseDTO> getCart(@AuthenticationPrincipal User springSecurityUser) {
        CartResponseDTO cartResponseDTO = controllerMapper.toCartResponseDTO(
                cartService.getCartForPrincipal(springSecurityUser)
        );
        return ResponseEntity.ok(cartResponseDTO);
    }

    @PostMapping("/items")
    public ResponseEntity<CartResponseDTO> addItemToCart(@AuthenticationPrincipal User springSecurityUser,
    ...
    public ResponseEntity<CartResponseDTO> updateCartItem(@AuthenticationPrincipal User springSecurityUser,
    ...
    public ResponseEntity<Void> removeItemFromCart(@AuthenticationPrincipal User springSecurityUser,
                                                   @PathVariable Long itemId) {
    ...
    @DeleteMapping
    public ResponseEntity<CartResponseDTO> clearCart(@AuthenticationPrincipal User springSecurityUser) {
        CartResponseDTO cartResponseDTO = controllerMapper.toCartResponseDTO(
                cartService.clearCartForPrincipal(springSecurityUser)
        );
        return ResponseEntity.ok(cartResponseDTO);
    }
}
